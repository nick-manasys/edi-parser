...
import com.mulesoft.flatfile.lexical.EdiConstants.ASCII_CHARSET
import com.mulesoft.flatfile.lexical.X12Constants._
import com.mulesoft.flatfile.lexical.X12Lexer.InterchangeStartStatus.VALID
import com.mulesoft.flatfile.schema.tools.DefaultX12EnvelopeHandler
import com.mulesoft.flatfile.schema.tools.DefaultX12NumberProvider
...
class X12SchemaParserWriterTests extends FlatSpec with Matchers with SchemaJavaDefs {
  ...
  it should "write the ISA envelope when initialized and then terminated" in {
    ...
    val config = X12WriterConfig(true, CharacterRestriction.BASIC, -1, ASCII_CHARSET, "*>U~", null)
  ...
  it should "roundtrip a parsed document" in {
    ...
    val out = new ByteArrayOutputStream
    val config = X12WriterConfig(true, CharacterRestriction.EXTENDED, -1, ASCII_CHARSET, "*>U~", null)
