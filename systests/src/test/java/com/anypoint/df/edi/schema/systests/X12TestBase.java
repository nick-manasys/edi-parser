...
public abstract class X12TestBase extends TestBase {
	...
	/**
	 * Mask variable values in an ISA segment by replacing them with 'X'
	 * characters.
	 * 
	 * @param offset
	 * @param datasep
	...
	/**
	 * Mask variable values in a GS segment by replacing them with 'X'
	 * characters, or in the case of the variable-length group control number
	 * just deleting the value.
	 * 
	 * @param offset
	 * @param datasep
	 * @param message
	...
	/**
	 * Prepare segments for comparison. This returns a list of segments with all
	 * the variable values masked out.
	 * 
	 * @param segterm
	 * @param datasep
	 * @param message
	...
	protected List<String> prepareSegments(char segterm, char datasep, String message) {
		...
		while ((split = message.indexOf(segterm, base)) > 0) {
			...
			} else if (seg.startsWith("ST" + datasep) || seg.startsWith("SE" + datasep) | seg.startsWith("GE" + datasep) | seg.startsWith("IEA" + datasep)) {
	...
	/**
	 * Parse a document, write out the parsed data as a new document, and
	 * compare the original and the written document.
	 * 
	 * @param path
	 * @throws IOException
	 */
	...
	/**
	 * Replace date/time fields in an acknowledgment with X characters, allowing
	 * it to be compared with a static string.
	 * 
	 * @param ack
	 * @return replaced
	 */
