...
public class StandardD95bTest extends EdifactTestBase {
    ...
    public void verifyBAPLIE() throws Exception {
        // this causes problems with segments reused within a loop
        loadSchema("/edifact/d95b/BAPLIE.esl");
        parseAndCheckWrite("/edifact/d95b/HPA-BAYPLAN.edi");
        assertEquals("UNB+UNOA:3+1STOP+FACTT+XXXXXXXXXXX+++COARRI'UNH++CONTRL:D:95B:UN'UCI+15731968+FACTT+1STOP+7'UNT+3+1'UNZ+1+1'",
    ...
    public void verifyCODECO() throws Exception {
        ...
        assertEquals("UNB+UNOA:3+1STOP+FACTT+XXXXXXXXXXX+++CODECO'UNH++CONTRL:D:95B:UN'UCI+15733961+FACTT+1STOP+7'UNT+3+1'UNZ+1+1'",
            stripAckDates(parseAndReturnAck("/edifact/d95b/CODECO.edi")));
        parseAndCheckWrite("/edifact/d95b/DEHIRE-CODECO.edi");
        assertEquals("UNB+UNOA:3+MAEU+APPLINK+XXXXXXXXXXX+'UNH++CONTRL:D:95B:UN'UCI+0023286999+APPLINK+MAEU+7'UNT+3+1'UNZ+1+1'",
